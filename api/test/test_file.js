...
describe('GET /toponimspartnom', function() {
  ...
  it('obtains 20 toponims, checks for presence of totalRecords', function(done) {
    chai.request('http://127.0.0.1:' + (process.env.RUNNING_PORT_TEST || '8080') + '/api')
      .get('/toponimspartnom')
      ...
      .end(function(err, res) {
        ...
        assert.ok(res.body.totalRecords != null, "totalRecords is missing!");
        done();
      });
  });


  it('obtains 20 toponims ordered desc, checks for presence of totalRecords', function(done) {
    chai.request('http://127.0.0.1:' + (process.env.RUNNING_PORT_TEST || '8080') + '/api')
      .get('/toponimspartnom')
      .set('x-access-token', token)
